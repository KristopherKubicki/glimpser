...
# does not actually update the database - just overrides the existing run
...
# Make sure we are properly in the application directory
...
if not os.path.exists(config.DATABASE_PATH):
    # if there is no database, initialize and start a new glimpser
    from generate_credentials import generate_credentials
    generate_credentials()

# Ensure logging directory exists
if not os.path.isdir(os.path.dirname(config.LOGGING_PATH)):
...
# Stream handler to log to the console if enabled
if args.console_log:
