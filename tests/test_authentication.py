...
class TestAuthentication(unittest.TestCase):
    ...
    def test_skip_login_for_allowed_subnet(self):
        with (
            ...
            patch("app.routes.config.LAN_GUEST_MODE", "full"),
        ):
            response = self.client.get(
    ...
    def test_lan_guest_read_only_blocks_post(self):
        with (
            patch(
                "app.routes.config.SKIP_LOGIN_SUBNETS",
                [ip_network("127.0.0.0/8")],
            ),
            patch("app.routes.config.LAN_GUEST_MODE", "read_only"),
        ):
            response = self.client.post(
                "/protected-post", environ_base={"REMOTE_ADDR": "127.0.0.1"}
            )
            self.assertEqual(response.status_code, 302)
            self.assertIn("/login", response.headers["Location"])

    def test_lan_guest_read_only_allows_get(self):
        with (
            patch(
                "app.routes.config.SKIP_LOGIN_SUBNETS",
                [ip_network("127.0.0.0/8")],
