...
class TestEmailAlerts(unittest.TestCase):
    def test_send_email_alert_disabled(self):
        with patch("app.utils.email_alerts.EMAIL_ENABLED", "False"), patch(
            "app.utils.email_alerts.EMAIL_SENDER", "sender@example.com"
        ), patch("app.utils.email_alerts.EMAIL_RECIPIENTS", "rec@example.com"), patch(
    ...
    def test_send_email_alert_enabled(self):
        ...
        with patch("smtplib.SMTP", return_value=mock_cm) as mock_smtp, patch(
            ...
            "app.utils.email_alerts.EMAIL_RECIPIENTS", "r1@example.com,r2@example.com"
        ), patch(
            "app.utils.email_alerts.EMAIL_SMTP_SERVER", "smtp.example.com"
        ), patch(
            "app.utils.email_alerts.EMAIL_SMTP_PORT", "587"
        ), patch(
            "app.utils.email_alerts.EMAIL_SMTP_TIMEOUT", 5
        ), patch(
            "app.utils.email_alerts.EMAIL_USE_TLS", "true"
        ), patch(
            "app.utils.email_alerts.EMAIL_USERNAME", "user"
        ), patch(
            "app.utils.email_alerts.EMAIL_PASSWORD", "pass"
