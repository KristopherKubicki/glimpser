...
### Problem: Restarting from the Settings page leaves Gunicorn hanging

Prior versions replaced the running Python process using `os.execv`. When
running under Gunicorn this only restarted a worker and the new process failed
to bind the port.

**Solution:**

- Glimpser now detects Gunicorn via the `SERVER_SOFTWARE` environment variable
  and simply exits. The master process then spawns a fresh worker.
- When running `python main.py` directly, the interpreter re-executes so the
