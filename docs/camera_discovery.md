...
Background discovery can run automatically every hour when the `DISCOVERY_AUTOSTART` setting is enabled. The search icon now appears only while a scan is running so you can see when discovery is active. It hides again once the scan finishes.
