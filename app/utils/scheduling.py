    ...
    for filename in os.listdir(directory):
        if filename.endswith('.png') and 'motion' in filename:
            ...
            try:
                ...
                time_diff = abs(last_caption_time - timestamp) # todo.. can't go over... 
        ...
        try:
          with Image.open(image_path) as image:  # consider unlinking if this fails to open
            ...
            if motion is True:
                ...
                draw.text((x, y), motion_icon, font=font, fill=(255, 255, 255, 255))  # White text
            ...
            if caption is not None:
                ...
                image.paste(background, (x - 10, y - 5), background)
                # Draw the timestamp in white text on the black transparent box
                draw.text((x, y), caption, font=font, fill=(255, 255, 255, 255))  # White text

            # Save the image
            image.save(image_path, 'PNG')
        except Exception as e:
          print("warning! weird image opening error")
...
def update_camera(name, template, image_file=None):
    ...
    if lsuc is True:
        ...
        png_files = [f for f in os.listdir(directory) if f.endswith('.png') and os.path.isfile(os.path.join(directory, f))]
        if not png_files:
        ...
        if len(png_files) > 1:
            percentage_difference = calculate_difference_fast(os.path.join(directory,png_files[-2]),os.path.join(directory,png_files[-1]))
        ...
        #  
        last_caption_time, last_motion_caption = None, None
        ...
        if True:
            ...
            try:
                last_caption_time = datetime.datetime.strptime(template.get('last_caption_time','1970-01-01 00:00:00'), "%Y-%m-%d %H:%M:%S")
                if last_caption_time and datetime.datetime.utcnow() - last_caption_time > datetime.timedelta(hours=ldelta): # one caption per day is fine otherwise...
                    allow = True
                    last_caption_trigger = True
                    #print("allowing from old caption", name)
            except Exception as e:
        ...
        if allow:
            ...
            if os.path.exists(os.path.join(directory, 'reference.png')): # if doesnt exist, consider taking the oldest?
            ...
            update_success = save_template(name, template)
...
def update_summary():
    ...
    sorted_templates = sorted(templates.items(), key=lambda item: item[1].get('last_caption_time', ''), reverse=True)
    ...
    for id, template in sorted_templates:
        ...
        if template.get('last_caption_time'):
            ...
            fnotes = re.split(r'\s*?(.+?[\?\!\.\,])(?: \s?|\t|$)', template.get('notes','').strip(), flags=re.DOTALL)
            gnotes = re.split(r'\s*?(.+?[\?\!\.\,])(?: \s?|\t|$)', template.get('last_caption','').strip(), flags=re.DOTALL)
            ...
            lstring += 'name: ' + name + '\tgroups: '+ template.get('groups','') + '\tupdated: ' + template.get('last_caption_time','') + '\tprompt: ' + str(fnotes) + '\tresponse: ' + str(gnotes) + '\n'
    ...
    lsuc = False
    for leach in re.findall(r'^\s*?`?`?`?j?s?o?n?\n?(\{.+?\})\n?`?`?`?',lsum, flags=re.DOTALL):  # if we don't find this, then we wasted money...
...
def schedule_summarization():
    ...
    try:
        scheduler.add_job(func=update_summary, trigger=CronTrigger(minute=0), id='summary', replace_existing=True)
    except Exception as e:
...
def schedule_crawlers():
    ...
    for index, (id, template) in enumerate(shuffled_templates):
        ...
        try:
            scheduler.add_job(func=update_camera, trigger='interval', seconds=seconds, start_date=datetime.datetime.now() + datetime.timedelta(seconds=offset_delay_seconds), args=[name, template], id=name, replace_existing=True)
        except Exception as e:
            print("job schedule error:", e)
    ...
    try:
        scheduler.add_job(func=init_crawl, trigger='date', run_date=datetime.datetime.now() + datetime.timedelta(minutes=3), id='init_crawl')
    except Exception as e:
