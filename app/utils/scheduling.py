...
def llava_caption(prompt, image_paths):
    """Return a caption using a lightweight local model.

    The real implementation should call a LLaVA-based captioning
    model. For testing purposes this simply falls back to
    ``chatgpt_compare`` so the scheduling logic can rely on it.
    """

    try:
        return chatgpt_compare(prompt, image_paths)
    except Exception:
        return None
...
def update_camera(name, template, image_file=None, motion=False):
    ...
    if lsuc is True:
        ...
        if allow:
            ...
            # Reload template to ensure latest metadata for captioning
            template = get_template(name)

            lprompt = ""
            if template.get("notes"):
                # separate notes from any autogenerated text
                lprompt += template["notes"].strip() + "\n"

            # Decide which caption method to use. When motion or object
            # filters triggered we prefer GPT, otherwise we fall back to a
            # cheaper LLaVA based caption.
            use_gpt = bool(lsum) or bool(object_filter)

            if not use_gpt:
                lret = llava_caption(lprompt, image_paths)
...
def schedule_crawlers():
    ...
    for index, (id, template) in enumerate(shuffled_templates):
        ...
        # When a template is updated the existing job may have outdated
        # settings. Remove any previous job so the new configuration is
        # scheduled correctly.
        try:
