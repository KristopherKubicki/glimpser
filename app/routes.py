...
def init_routes(app: Flask) -> None:
    ...
    @app.route("/story_video", methods=["POST"])
    @login_required
    @limit_rate(30)
    def story_video():
        """Return a short caption story video generated from provided frames.

        The request body may contain ``frames`` with image paths, captions and
        per-frame durations generated by an LLM. If ``frames`` is not provided,
        ``images`` and ``captions`` lists are used with a default duration.
        """

        data = request.get_json(force=True) or {}
        frames = data.get("frames")
        if frames:
            images = [f.get("image") for f in frames]
            captions = [f.get("caption", "") for f in frames]
            durations = [f.get("duration", 3) for f in frames]
        else:
            images = data.get("images") or []
            captions = data.get("captions") or []
            durations = None

        if not images or not captions or len(images) != len(captions):
            return (
        ...
        with tempfile.NamedTemporaryFile(suffix=".mp4", delete=False) as tmp:
            success = video_archiver.compile_caption_story(
                images, captions, tmp.name, durations=durations
            )
            if not success:
                return (
                    jsonify({"status": "error", "message": "Compilation failed"}),
                    500,
                )
            return send_file(tmp.name, mimetype="video/mp4")
