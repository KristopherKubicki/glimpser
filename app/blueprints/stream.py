...
def create_blueprint() -> Blueprint:
    ...
    def stream_png():
        ...
        if camera:
            ...
            if os.path.exists(cam_path) and routes.screenshots._is_valid_png(cam_path):
        ...
        if group:
            ...
            if os.path.exists(group_path) and routes.screenshots._is_valid_png(
                group_path
            ):
        ...
        if (
            routes.last_time
            and time.time() - routes.last_time < 1
            and routes.last_shot
            and os.path.exists(routes.last_shot)
        ):
        ...
        if most_recent_file is None:
            if routes.last_shot and os.path.exists(routes.last_shot):
                return routes.send_conditional_file(
                    routes.last_shot, routes.PNG_TTL_SEC
                )
            routes.abort(404)

        routes.last_time = time.time()
        routes.last_shot = most_recent_file

        if os.path.exists(most_recent_file) and routes.screenshots._is_valid_png(
            most_recent_file
        ):
    ...
    @routes.login_required
    def handle_rtsp():
        session_id = routes.request.headers.get("Session", str(uuid.uuid4()))
        ...
        if routes.request.method == "OPTIONS":
            ...
            if session_id not in routes.rtsp_sessions:
                routes.abort(454)
            routes.rtsp_sessions[session_id]["last_keepalive"] = time.time()
            return Response(headers={"CSeq": cseq, "Session": session_id})
    ...
    def test_pattern_mjpg():
        ...
        return Response(
            generate_pattern(), mimetype="multipart/x-mixed-replace; boundary=frame"
        )
    ...
    @routes.login_required
    def stream():
        return routes.render_template("stream.html", page_title="Stream")
