...
SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine) # settings only tthread

def get_setting(name, default=None):

    session = SessionLocal()
    try:
        result = session.execute(text("SELECT value FROM settings WHERE name = '%s'" % name)).fetchone()
